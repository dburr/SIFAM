...
public class BackupDatabase{
    ...
    public BackupDatabase(String name) throws Exception{
        databaseName = name;
        dbHelper = new backup_dbHelper(SIFAM.getContext());
    };

    public class backup_dbHelper extends SQLiteOpenHelper {
        public static final int DATABASE_VERSION = 1;
        public backup_dbHelper(Context context) {
            super(context, Environment.getExternalStorageDirectory() + "//SIFAM//Backups//" + databaseName + ".db", null, DATABASE_VERSION);
            SIFAM.log("dbHelper " + databaseName);
        }
    ...
    public ArrayList<Account> getFolders(long folder) {
        SIFAM.log("BackupDatabase > getFolders (From Folder "+ folder + ")");
        ...
        try {
            ...
            SIFAM.log(SIFAM.exceptionToString(ex));
        }
        return folders;
    }
        ...
        try {
            ...
            if (c.moveToFirst()) {
                while (c.isAfterLast() == false) {
                    ...
                    a.userKey = c.getString(c.getColumnIndex(Database.Accounts.cUser));
                    a.userPass = c.getString(c.getColumnIndex(Database.Accounts.cPass));
                    accountList.add(a);
                    c.moveToNext();
                }
            }
