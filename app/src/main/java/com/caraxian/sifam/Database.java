...
public final class Database {
    ...
    public long saveNewAccount(String name, String user, String pass, String server, long folder) {
        if (user.length() == 0) {
    ...
    public int countAccounts(long folder, String search, boolean searchAll) {
        ...
        for (Server s : SIFAM.serverList) {
            if (s.enabled == true) {
                ...
                selection += Accounts.cServer + "='" + s.code + "'";
            }
        }
    ...
    public ArrayList<Account> getAccounts(long folder, Integer limit, Integer offset, String sortby, boolean reverse, String search, boolean searchAll) {
        ...
        if (SIFAM.FOLDERS_ON_BOTTOM == false && search.equals("")) {
            ...
            if (c.moveToFirst()) {
                while (c.isAfterLast() == false) {
                    ...
                    if (a.userKey.length() > 0) {
                        ...
                        SIFAM.log("Invalid Account Deleted: id " + a.id);
                        deleteAccount(a);
                    }
        ...
        if (SIFAM.FOLDERS_ON_BOTTOM == true && search.equals("")) {
            accountList.addAll(getFolders(folder));
        }
        return accountList;
    }
